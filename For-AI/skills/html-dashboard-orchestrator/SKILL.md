...
3. Keep the UI lab-oriented.
   ...
   - Treat each Trial Designer row as one row-level trial type: all fixed/randomizer blocks in the row define that unique `trial_type_label`, and rows are scheduled sequentially top-to-bottom inside the block. Keep visible labels aligned with this mental model even if internal schema names still use `trial_strips`.
