...
The dashboard should behave as a one-page local app: a fixed floating left rail, smooth section navigation, and user-adjustable panel sizing through direct workspace splitters/gutters and panel-edge handles rather than left-rail sliders. Custom mode is not an open-ended freeform page; selecting `Custom design (define manually)` should force the researcher through Study Profile, Stimulus Design, Trial Assembly, Baseline Strategy, Run Preparation, and Review in order. Render/prepare/run actions must stay locked until the custom design contains the minimum runnable profile fields.

The HTML dashboard should be organized as sequential decision segments rather than a mixed dashboard. Segment 1 is the profile decision: published preload versus custom design and study-level naming. Segment 2 is the looming stimuli builder: trajectory numeric controls, 2D/3D preview, source staging/baking, baked-source lists, and backend feedback about local bake/render progress. Segment 3 is the trial designer: within-block event sequences, SOAs, repetitions, blocks, catch percentage, and trial preview. Segment 4 is baseline strategy: no baseline, tactile-only timing anchors, SOA 0, sound-offset/end anchors, or custom baseline timings plus baseline proportion, live baseline count feedback, and whole-experiment duration estimates. Filmstrip/event-sequence rows are sequential events inside a block, not alternate block definitions. Participant counts, participant ID, session preparation, participant order preview, jobs, and review stay at the bottom in the run/session area.
